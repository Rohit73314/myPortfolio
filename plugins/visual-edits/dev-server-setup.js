...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
     ...
     // Allow extra domains via env (comma-separated)
const EXTRA_ALLOWED_DOMAINS = process.env.ALLOWED_ORIGINS
...
const isAllowedOrigin = (origin) => {
  ...
  return EXTRA_ALLOWED_DOMAINS.some((domain) =>
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(
                `git -c user.name="visual-edit" -c user.email="visual-edit@localhost" add "${targetFile}"`
              );
              execSync(
                `git -c user.name="visual-edit" -c user.email="visual-edit@localhost" commit -m "visual_edit_${timestamp}"`
              );
